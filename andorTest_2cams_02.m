...
% Define parameters
%
NumFrames = 800;     % Number of scans in kinetic series
ExpTime = 0.0001;      % Exposure time
...
numVidFrames = 10; % Number of frames to show in video mode before exiting
showFramenum = true;
...
%               disp(nImAcq.value)
    ...
    case 4
        ...
        for ii = 1:numVidFrames
            ...
            imagesc(imPtr.value,'Parent',ax1)
            axis square
            
            error=calllib('libandor','SetCurrentCamera',cam2Handle);
            getimerr=calllib('libandor','GetMostRecentImage',imPtr,npixels)
            imagesc(imPtr.value,'Parent',ax2)
            axis square
